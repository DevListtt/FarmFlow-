    ...
    <Layout>
      ...
      <div className="grid grid-cols-1 gap-6 xl:grid-cols-3">
        <section className="card p-6 xl:col-span-2">
          <h2 className="text-xl font-semibold text-brand-text">Fonctions livrées</h2>
          <div className="mt-4 grid gap-3 md:grid-cols-2">
            {actions.map((action) => (
              <article key={action.title} className="rounded-xl border border-brand-border p-4">
                <div className="flex items-center justify-between gap-3">
                  <h3 className="font-semibold text-brand-text">{action.title}</h3>
        ...
        <aside className="card p-6">
          <h2 className="text-xl font-semibold text-brand-text">Endpoint principal</h2>
          <code className="mt-4 block rounded-lg bg-slate-900 p-3 text-sm text-white">{endpoint}</code>
          <div className="mt-5 space-y-2">
            {responseFields.map((field) => (
              <div key={field} className="rounded-lg bg-brand-surface px-3 py-2 text-sm text-brand-text">{field}</div>
            ))}
          </div>
        </aside>
      </div>
      ...
      {payloadExample && (
        <section className="card mt-6 p-6">
          <h2 className="text-xl font-semibold text-brand-text">Exemple de payload</h2>
